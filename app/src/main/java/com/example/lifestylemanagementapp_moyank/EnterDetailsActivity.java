...
public class EnterDetailsActivity extends AppCompatActivity {
    ...
    AutoCompleteTextView FoodAutoCompleteTextView;
    ArrayAdapter FoodList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FoodAutoCompleteTextView = findViewById(R.id.FoodAutoCompleteTextView);
        ...
        final DatabaseHelperClass databaseHelperClass = new DatabaseHelperClass(EnterDetailsActivity.this);
        final UserDataBaseHelperClass userDataBaseHelperClass = new UserDataBaseHelperClass(EnterDetailsActivity.this);
        List<CustomerModel> databaseList = databaseHelperClass.getCompleteDatabase();
        List<String> FoodListFromDatabase = databaseHelperClass.getFoodNameColumnOfDatabase();
        FoodList = new ArrayAdapter<String>(this, android.R.layout.select_dialog_item, FoodListFromDatabase);
        FoodAutoCompleteTextView.setAdapter(FoodList);
        FoodAutoCompleteTextView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
        ...
        AddCaloriesButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if ((SelectedItem != null) && (finalDate != null)) {
                    Integer Quantity;
                    List<CustomerModel> SelectedFoodItemFromDatabase = databaseHelperClass.getSpecificData(SelectedItem);
                    ...
                    String cal = SelectedFoodItemFromDatabase.get(0).getCalories();
                    if (cal.contains("cal")) {
                        cal = cal.replaceAll("cal", "");
                        cal = cal.replaceAll("\\s", "");
                    }
                    Integer CaloriesInItem = Integer.parseInt(cal);
                    try {
                        Quantity = Integer.parseInt(QuantityEditText.getText().toString());
                    }
                    catch (Exception e){
                        Toast.makeText(EnterDetailsActivity.this, "Some Error has occurred...", Toast.LENGTH_SHORT).show();
                        Quantity = 0;
                    }
                    DecimalFormat f = new DecimalFormat("##.00");
                    Double TotalCaloriesIntake = CaloriesInItem * Double.parseDouble(f.format(Quantity / 100.0));
                    CalorieIntake += TotalCaloriesIntake;
                    ...
                    if (userDataBaseHelperClass.CheckIsDataAlreadyInDBorNot("ENTRY_DATE", finalDate)) {
                        userDataBaseHelperClass.UpdateCalorieIntakeInDatabase(finalDate, CalorieIntake);
                        Toast.makeText(EnterDetailsActivity.this, "Data updated ", Toast.LENGTH_SHORT).show();
                        ...
                        Toast.makeText(EnterDetailsActivity.this, "Data added", Toast.LENGTH_SHORT).show();
                    }
                    ...
                    Toast.makeText(EnterDetailsActivity.this, "Please input all values and retry...", Toast.LENGTH_LONG).show();
            }
        });
